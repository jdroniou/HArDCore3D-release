...
//    _assembly_time = timer.elapsed().user + timer.elapsed().system;
    ...
    if (BC==0){
      ...
      for (size_t ibF = 0; ibF < mesh->n_b_faces(); ibF++){
        Face* face = b_faces[ibF]; 
	      ...
	      auto quadF = generate_quadrature_rule(*face, 2*hho.K());
  ...
  Eigen::MatrixXd HHO_LocVarDiff::diffusion_operator(HybridCore &hho, const size_t iT, const ElementQuad &elquad) const {
    ...
    std::vector<Eigen::ArrayXXd> vec_phiT_quadT = elquad.get_vec_phiT_quadT(hho.K());
    ...
    std::transform(quadT.begin(), quadT.end(), kappaT_quadT.begin(),
		   [this,&cell](QuadratureNode qr) -> Eigen::MatrixXd { return kappa(qr.x, qr.y, qr.z, cell); });
    ...
    std::vector<Eigen::MatrixXd> MFF(nfacesT, Eigen::MatrixXd::Zero(hho.nlocal_face_dofs(), hho.nlocal_face_dofs()));
    std::vector<Eigen::MatrixXd> MFT(nfacesT, Eigen::MatrixXd::Zero(hho.nlocal_face_dofs(), hho.nhighorder_dofs()));
    std::vector<Eigen::MatrixXd> MTT_on_F(nfacesT, Eigen::MatrixXd::Zero(dimPKcell, hho.nlocal_cell_dofs()));
	...
	RHS_GT.block(r*dimPKcell, offset_F, dimPKcell, hho.nlocal_face_dofs()) += 
	  nTF(r) * (MFT[ilF].topLeftCorner(hho.nlocal_face_dofs(), dimPKcell)).transpose();
      }
    }
...
//		double dTF = cell->face(ilF)->diam();
    ...
    Eigen::Vector3d xF = cell->face(ilF)->center_mass();
      ...
      auto kappa_TF = kappa(xF.x(), xF.y(), xF.z(), cell).trace();
    ...
    Face* face = cell->face(ilF);
	  ...
	  if (cell->face(ilF)->is_boundary()){
	    ...
	    for (size_t i = 0; i < hho.nlocal_face_dofs(); i++){
	      ...
	      auto quadF = generate_quadrature_rule(*face, _deg_kappa+ 2*hho.K()+2);
