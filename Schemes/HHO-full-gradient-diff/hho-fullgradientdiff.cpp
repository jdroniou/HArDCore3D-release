...
int main(int argc, const char* argv[])
{
  ...
  // Build the mesh and reorder faces to handle Dirichlet BCs (Dirichlet faces at the end -- pure Dirichlet for the moment)
                                          ...
                                          )
{
  ...
  for (size_t idF = 0; idF < m_BC.n_dir_faces(); idF++){
    ...
    QuadratureRule quadF = generate_quadrature_rule(*F, 2*m_hhospace.degree()+2);
    
    auto phiF_quadF = evaluate_quad<Function>::compute(*m_hhospace.faceBases(idF).Polyk, quadF);
    UDir.segment(idF * m_hhospace.numLocalDofsFace(), m_hhospace.numLocalDofsFace()) 
        = l2_projection<HHOSpace::PolyBasisFaceType>(u, *m_hhospace.faceBases(idF).Polyk, quadF, phiF_quadF);
  } // for idF
