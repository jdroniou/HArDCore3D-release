...
int main(int argc, const char* argv[]) {
  ...
  // Diffusion tensor
  HHO_Diffusion::tensor_function_type kappa = [&](double x, double y, double z, Cell* cell) {
						return tcase.diff(x,y,z,cell);
					      };

  // Source term
  HHO_Diffusion::source_function_type source = [&](double x, double y, double z, Cell* cell) {
						 return tcase.source(x,y,z,cell);
					       };

  // Exact solution and gradient
  HHO_Diffusion::solution_function_type exact_solution = [&](double x, double y, double z) {
							   return tcase.sol(x,y,z);
							 };
  HHO_Diffusion::grad_function_type grad_exact_solution = [&](double x, double y, double z, Cell* cell) {
  ...
  //	double average_exact_sol = hho.integrate_over_domain([&](auto x, auto y, auto z) {
