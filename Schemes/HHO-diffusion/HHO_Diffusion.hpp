...
namespace HArDCore3D {
  ...
  class HHO_Diffusion {
    ...
    // Types
  public:
    using solution_function_type = std::function<double(double,double,double)>;		///< type for solution
    using source_function_type = std::function<double(double,double,double,Cell*)>;		///< type for source
    using grad_function_type = std::function<Eigen::Vector3d(double,double,double,Cell*)>;		///< type for gradient
    using tensor_function_type = std::function<Eigen::Matrix3d(double,double,double,Cell*)>;		///< type for diffusion tensor
  ...
  Eigen::VectorXd HHO_Diffusion::solve(HybridCore &hho) {
    ...
    // System matrix
    Eigen::SparseMatrix<double> GlobMat(hho.ntotal_face_dofs(), hho.ntotal_face_dofs());
    ...
    Eigen::SparseMatrix<double> ScBeMat(hho.ntotal_cell_dofs(), hho.ntotal_face_dofs());
    ...
    Eigen::VectorXd GlobRHS = Eigen::VectorXd::Zero(hho.ntotal_face_dofs());
    ...
    for (size_t iT = 0; iT < mesh->n_cells(); iT++) {
      ...
      size_t face_dofs = nlocal_faces * hho.nlocal_face_dofs();
      ...
      _itime[5] += timerint.elapsed().user + timerint.elapsed().system;
      ...
      Eigen::VectorXd bF = Eigen::VectorXd::Zero(face_dofs);
	...
	Eigen::MatrixXd ATF = aT[iT].topRightCorner(hho.nlocal_cell_dofs(), face_dofs);
	...
	bF = bT.tail(face_dofs) - ATF.transpose() * invATT_bTcell;
	...
	for (size_t i = 0; i < hho.nlocal_cell_dofs(); i++) {
	  for (size_t jlF = 0; jlF < nlocal_faces; jlF++) {
	    ...
	    for (size_t jk = 0; jk < hho.nlocal_face_dofs(); jk++) {
	      const size_t jLocal = jlF * hho.nlocal_face_dofs() + jk;
	      const size_t jGlobal = jF * hho.nlocal_face_dofs() + jk;
	...
	Eigen::Vector3d xT = iCell->center_mass();
	double phiT_cst = hho.cell_basis(iT,0)(xT.x(), xT.y(), xT.z());
	for (size_t ilF = 0; ilF < nlocal_faces; ilF++){
	  Eigen::Vector3d xF = iCell->face(ilF)->center_mass();
	  size_t iF = iCell->face(ilF)->global_index();
	  double phiF_cst = hho.face_basis(iF,0)(xF.x(), xF.y(), xF.z());
	...
	for (size_t jlF = 0; jlF < nlocal_faces; jlF++) {
	  const size_t jF = iCell->face(jlF)->global_index();
	  const size_t jGlobal = jF * hho.nlocal_face_dofs();
	...
	for (size_t ik = 0; ik < hho.nlocal_face_dofs(); ik++) {
	  const size_t iLocal = ilF * hho.nlocal_face_dofs() + ik;
	  const size_t iGlobal = iF * hho.nlocal_face_dofs() + ik;
	  for (size_t jlF = 0; jlF < nlocal_faces; jlF++) {
	    const size_t jF = iCell->face(jlF)->global_index();
	    for (size_t jk = 0; jk < hho.nlocal_face_dofs(); jk++) {
	      const size_t jLocal = jlF * hho.nlocal_face_dofs() + jk;
    ...
    if (BC==0){
      ...
      nb_unknowns = hho.ninternal_face_dofs();
      nb_fixed_dofs = hho.nboundary_face_dofs();
      A = GlobMat.topLeftCorner(nb_unknowns, nb_unknowns);
		
      // Boundary value: UDir corresponds to the L2 projection of the exact solution on the polynomial spaces on the faces
      UDir = Eigen::VectorXd::Zero(nb_fixed_dofs);
      std::vector<Face *> b_faces = mesh->get_b_faces(); // List of boundary faces

      for (size_t ibF = 0; ibF < mesh->n_b_faces(); ibF++){
        Face* face = b_faces[ibF];
	      size_t iF = face->global_index();
	      // Mass matrix and boundary values
	      QuadratureRule quadF = generate_quadrature_rule(*face, 2*hho.K());
	      std::vector<Eigen::ArrayXd> phiF_quadF = hho.basis_quad("face", iF, quadF, hho.K());
	      Eigen::MatrixXd MFF = hho.gram_matrix(phiF_quadF, phiF_quadF, hho.nlocal_face_dofs(), hho.nlocal_face_dofs(), quadF, true);

	      // Compute (uexact, phi_i)_F for all face basis functions phi_i
	      Eigen::VectorXd RHS_UDirF = Eigen::VectorXd::Zero(hho.nlocal_face_dofs());
	      for (size_t j = 0; j < hho.nlocal_face_dofs(); j++){
	        const auto& phi_j = hho.face_basis(iF, j);
	        QuadratureRule quadF = generate_quadrature_rule(*face, 2*hho.K()+2);
	        for (QuadratureNode qF : quadF){
	          RHS_UDirF(j) += qF.w * phi_j(qF.x, qF.y, qF.z) * exact_solution(qF.x, qF.y, qF.z);
	        }
	      }
	      // Project exact solution
	      UDir.segment(ibF * hho.nlocal_face_dofs(), hho.nlocal_face_dofs()) = MFF.ldlt().solve(RHS_UDirF);
      ...
      nb_unknowns = hho.ntotal_face_dofs();
      A = GlobMat;
      B = GlobRHS;
      UDir = Eigen::VectorXd::Zero(nb_fixed_dofs);
    }
    ...
    if (hho.L()>=0) {
      Xh.head(hho.ntotal_cell_dofs()) = ScRHS - ScBeMat * Xh.tail(hho.ntotal_face_dofs());
    } else {
      Xh.head(hho.ntotal_cell_dofs()) = ScBeMat * Xh.tail(hho.ntotal_face_dofs());
    }
    ...
    if (BC==1){
      ...
      auto average_exact_sol = hho.integrate_over_domain([&](auto x, auto y, auto z) {
      ...
      std::function<double(double, double, double)> AveDiff = [&average_exact_sol,&average](double x, double y, double z)->double
  ...
  Eigen::MatrixXd HHO_Diffusion::diffusion_operator(HybridCore &hho, const size_t iT, const ElementQuad &elquad) const {
    ...
    size_t local_dofs = hho.nlocal_cell_dofs() + nfacesT * hho.nlocal_face_dofs();

    //-------------------  Initialisatons ------------------------------//

    // Diffusion in the cell.
    std::function<Eigen::Matrix3d(double,double,double)> kappaT = [&](double x, double y, double z){
								    // Constant in the cell
								    Eigen::Vector3d cellcenter = cell->center_mass();
								    return kappa(cellcenter.x(), cellcenter.y(), cellcenter.z(), cell);
								    ...
								    // If the diffusion is piecewise constant, choosing the previous version ensures that, for face integrals, it's the value inside the cell that is computed (essential in case kappa is discontinuous across the faces)
								    //			return kappa(x, y, z, cell);
								  };

    // Cell quadrature nodes and values of cell basis functions and gradients at these nodes
    ...
    std::vector<Eigen::Matrix3d> kappaT_quadT(quadT.size());
    std::transform(quadT.begin(), quadT.end(), kappaT_quadT.begin(),
		   [&kappaT,&cell](QuadratureNode qr) -> Eigen::MatrixXd { return kappaT(qr.x, qr.y, qr.z); });
    ...
    std::vector<Eigen::MatrixXd> MFF(mesh->cell(iT)->n_faces(), Eigen::MatrixXd::Zero(hho.nlocal_face_dofs(), hho.nlocal_face_dofs()));
    std::vector<Eigen::MatrixXd> MFT(mesh->cell(iT)->n_faces(), Eigen::MatrixXd::Zero(hho.nlocal_face_dofs(), hho.nhighorder_dofs()));
    ...
    for (size_t ilF = 0; ilF < mesh->cell(iT)->n_faces(); ilF++) {
      // Offset for face unknowns
      const size_t offset_F = hho.nlocal_cell_dofs() + ilF * hho.nlocal_face_dofs();
      const auto& nTF = mesh->cell(iT)->face_normal(ilF);
      ...
      std::vector<Eigen::Vector3d> quadF_kappaT_nTF(nbqF, Eigen::Vector3d::Zero());
      for (size_t iqn = 0; iqn < nbqF; iqn++){
	quadF_kappaT_nTF[iqn] = quadF[iqn].w * kappaT(quadF[iqn].x, quadF[iqn].y, quadF[iqn].z)*nTF;
      }
	...
	for (size_t iqn = 0; iqn < nbqF; iqn++){
      ...
      MFF[ilF] = hho.gram_matrix(phiF_quadF, phiF_quadF, hho.nlocal_face_dofs(), hho.nlocal_face_dofs(), quadF, true);
      MFT[ilF] = hho.gram_matrix(phiF_quadF, phiT_quadF, hho.nlocal_face_dofs(), hho.nhighorder_dofs(), quadF, false);
    ...
    for (size_t ilF = 0; ilF < nfacesT; ilF++) {
      ...
      auto kappa_TF = kappaT(xF.x(), xF.y(), xF.z()).trace();

      // Face residual delta_TF^k = pi_F^k (rT uT) - u_F
      Eigen::MatrixXd MFFinv = MFF[ilF].inverse();
      Eigen::MatrixXd deltaTFK = MFFinv * MFT[ilF] * PT;
      deltaTFK.block(0, hho.nlocal_cell_dofs() + ilF * hho.nlocal_face_dofs(), hho.nlocal_face_dofs(), hho.nlocal_face_dofs()) -=
        Eigen::MatrixXd::Identity(hho.nlocal_face_dofs(), hho.nlocal_face_dofs());

      // Stabilisation term
      Eigen::MatrixXd deltaTFK_minus_deltaTL = deltaTFK - MFFinv * MFT[ilF].topLeftCorner(hho.nlocal_face_dofs(), hho.nlocal_cell_dofs()) * deltaTL;
  ...
  Eigen::VectorXd HHO_Diffusion::load_operator(HybridCore &hho, const size_t iT, const ElementQuad &elquad) const {
    ...
    const auto mesh = hho.get_mesh_ptr();
    Cell* cell = mesh->cell(iT);
    size_t cell_face_dofs = hho.nlocal_cell_dofs() + cell->n_faces()*hho.nlocal_face_dofs();
    ...
    for (size_t iqn = 0; iqn < nbq; iqn++){
      weight_source_quad(iqn) = quadT[iqn].w * source(quadT[iqn].x, quadT[iqn].y, quadT[iqn].z, cell);
    }
    ...
    if (cell->is_boundary()){
      if (BC==0){
	// Dirichlet BCs: no source terms on these faces
      } else if (BC==1) {
	// Neumann BCs
    ...
    Face* face = cell->face(ilF);
	  ...
	  if (cell->face(ilF)->is_boundary()){
	    // Offset for face unknowns
	    const size_t offset_F = hho.nlocal_cell_dofs() + ilF * hho.nlocal_face_dofs();
	    // Normal to the face
	    const auto& nTF = cell->face_normal(ilF);
	    // for each DOF of the boundary face
	    for (size_t i = 0; i < hho.nlocal_face_dofs(); i++){
	      const auto& phi_i = hho.face_basis(iF, i);
	      auto quadF = generate_quadrature_rule(*face, 2*hho.K()+2);
	      std::function<double(double,double,double)> Kgrad_n = [&](double x, double y, double z){
								      return nTF.dot(kappa(x,y,z,cell) * grad_exact_solution(x,y,z,cell)) * phi_i(x,y,z);
								    };
	      for (QuadratureNode qF : quadF){
		b(offset_F + i) += qF.w * Kgrad_n(qF.x, qF.y, qF.z);
	      }
	    }
	  }
	}
