...
int main(int argc, const char* argv[])
{
  ...
  uph.head(st.vCurl().dimension() - st.nbSCDOFs_u()) = uplambdah_condensed.head(st.vCurl().dimension() - st.nbSCDOFs_u()); 
  uph.segment(st.vCurl().dimension() - st.nbSCDOFs_u(), st.nbSCDOFs_u()) = sc_unknowns.head(st.nbSCDOFs_u());
  uph.segment(st.vCurl().dimension(), st.vGrad().dimension() - st.nbSCDOFs_p()) 
      = uplambdah_condensed.segment(st.vCurl().dimension() - st.nbSCDOFs_u(), st.vGrad().dimension() - st.nbSCDOFs_p());
...
Stokes::Stokes(
  ...
  : m_vemcore(vemcore),
    ...
    m_nloc_sc_u( m_vcurl.numLocalDofsCell() ),
    m_nloc_sc_p( m_vgrad.numLocalDofsCell() ),
                                  ...
                                  )
{
  ...
  std::tie(m_A, m_b, m_sc_A, m_sc_b) = parallel_assembly_system(m_vemcore.mesh().n_cells(), this->sizeSystem(), std::make_pair(this->nbSCDOFs(), this->sizeSystem()), this->nbSCDOFs(), assemble_all, m_use_threads);
  ...
  for (auto iF : m_vemcore.mesh().get_b_faces()) {
    ...
    // Boundary condition on the normal component of the velocity    
    {
      ...
      size_t dim_vcurl = m_vcurl.dimension() - nbSCDOFs_u();  // Gradients DOFs start here after SC
      for (size_t i = 0; i < I_F.size(); i++) {
...
LocalStaticCondensation Stokes::_compute_static_condensation(const size_t & iT) const
{
  ...
  // Dimensions
  size_t dim_u = m_vcurl.dimensionCell(iT) - m_nloc_sc_u;     // number of velocity unknowns after SC
  size_t dim_p = m_vgrad.dimensionCell(iT) - m_nloc_sc_p;     // number of pressure unknowns after SC
  ...
  auto IT_vcurl = m_vcurl.globalDOFIndices(T);
  auto IT_vgrad = m_vgrad.globalDOFIndices(T);
  auto it_IT_sys = std::copy(IT_vcurl.begin(), IT_vcurl.begin()+dim_u, IT_sys.begin()); // put skeletal DOFs of u in IT_sys
  size_t offset = m_vcurl.dimension() - nbSCDOFs_u();     // nb total of skeletal DOFs for u (where global skeletal dofs of p start)
  std::transform(IT_vgrad.begin(), IT_vgrad.begin()+dim_p, it_IT_sys, [&offset](const size_t & index) { return index + offset; });
  ...
  if (dim_sc>0){
    auto it_IT_sc = std::transform(IT_vcurl.begin()+dim_u, IT_vcurl.end(), IT_sc.begin(), [&offset](const size_t & index) { return index - offset; });
    size_t end_u = nbSCDOFs_u();
    offset = (m_vgrad.dimension() - nbSCDOFs_p());
    std::transform(IT_vgrad.begin()+dim_p, IT_vgrad.end(), it_IT_sc, [&offset, &end_u](const size_t & index) { return index - offset + end_u; });
  }  
