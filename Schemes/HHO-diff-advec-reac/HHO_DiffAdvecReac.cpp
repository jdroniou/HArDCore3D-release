...
int main(const int argc, const char *argv[])
{
    ...
    HHO3D model(hho, L, K, use_threads);
    ...
    std::cout << "     Mesh = " << mesh_name << ", nb cells= " << mesh_ptr->n_cells() << ", nb faces = " << mesh_ptr->n_faces() << "\n";
    std::cout << "     Degrees: face = " << K << "; cell = " << L << "\n";
    std::cout << "     Using threads = " << (use_threads ? "true" : "false") << "; Export matrix = " << (export_matrix ? "true" : "false") << "\n";

    const size_t n_local_face_dofs = DimPoly<Face>(K);
    ...
    const MatrixFType AT = [&](Cell *cell, ElementQuad &elquad) -> Eigen::MatrixXd {
        ...
        std::vector<Eigen::MatrixXd> MFF(n_local_faces, Eigen::MatrixXd::Zero(n_local_face_dofs, n_local_face_dofs));
        std::vector<Eigen::MatrixXd> MFT(n_local_faces, Eigen::MatrixXd::Zero(n_local_face_dofs, n_local_highorder_dofs));
        ...
        for (size_t iTF = 0; iTF < n_local_faces; iTF++)
        {
            ...
            if (!advec_zero)
            {
                ...
                BG.block(0, offset_F, n_local_cell_dofs, n_local_face_dofs) = compute_weighted_gram_matrix(advec_dot_nTF, phiT_quadF, phiF_quadF, quadF, n_local_cell_dofs, n_local_face_dofs);
            }
            ...
            BP.block(0, offset_F, n_local_highorder_dofs, n_local_face_dofs) = compute_weighted_gram_matrix(diff_nTF, dphiT_quadF, phiF_quadF, quadF);
            ...
            //     BP.row(i).segment(offset_F, n_local_face_dofs) += integrate(diff_dphiT_quadF_dot_nTF, phiF_quadF, quadF).transpose();
            // }
        }
        ...
        {
            const size_t offset_F = n_local_cell_dofs + iTF * n_local_face_dofs;
            // Operator to transform an element from the local space of unknowns to an element on a face
            Eigen::MatrixXd ID_FACE = Eigen::MatrixXd::Zero(n_local_face_dofs, n_local_dofs);
            ID_FACE.block(0, offset_F, n_local_face_dofs, n_local_face_dofs) = Eigen::MatrixXd::Identity(n_local_face_dofs, n_local_face_dofs);

            const VectorRd xF = cell->face(iTF)->center_mass();

            const VectorRd &nTF = cell->face_normal(iTF);
            // diffTF = k_Tn.n
            const double diffTF = (diff(xF, cell) * nTF).dot(nTF);
            const double hf = cell->face(iTF)->diam();
            ...
            if (!advec_zero)
            {
                ...
                Eigen::MatrixXd ID_MINUS_MFF_INV_MFT = ID_FACE - MFF_INV_MFT.topLeftCorner(n_local_face_dofs, n_local_cell_dofs) * ID_CELL;
                ADVECSTAB += advecTF * ID_MINUS_MFF_INV_MFT.transpose() * MFF[iTF] * ID_MINUS_MFF_INV_MFT;
            }

            // Diffusion stabilisation
            Eigen::MatrixXd DTF_MINUS_MFF_INV_MFT_DT = MFF_INV_MFT * PT - ID_FACE - MFF_INV_MFT.topLeftCorner(n_local_face_dofs, n_local_cell_dofs) * DT;
    ...
    model.set_dirichlet(exact_solution, BC.n_dir_faces());
