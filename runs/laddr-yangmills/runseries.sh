...
# Directories
#executable_name="laddr-maxwell"
#executable_name="laddr-maxwell-lm"
# executable_name="laddr-yangmills"
executable_name="laddr-yangmills-lm"
# executable_name="laddr-yangmills-lm-zeroavg"
...
do
    ...
    echo -e "------------------------------------------------------------------------------"
    # Execute code
    if($executable -m $meshfile -k $k -s $tcsol -x $stab_par -n ${iter[$j]} -v $stop -f $t_final -t $theta -l $nc -c $compute_cn -i $itersolver -q $constr_IC); then
      # Move outputs
