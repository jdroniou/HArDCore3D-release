#!/bin/bash
#
# Transform the meshes as per the require series
#

##############
## Series of required transformations
trans[1]="move-vertices"
trans[2]="make-flat-faces"
trans[3]="rf-to-vtu"
trans[4]="rf-to-fvca10format"
...
case ${mesh_family} in
    Tetgen-Cube-0)
	mesh[1]="RF:Tetgen-Cube-0/RF_fmt/cube.2"
	mesh[2]="RF:Tetgen-Cube-0/RF_fmt/cube.3"
	mesh[3]="RF:Tetgen-Cube-0/RF_fmt/cube.4"
#	mesh[4]="RF:Tetgen-Cube-0/RF_fmt/cube.5"
#	mesh[5]="RF:Tetgen-Cube-0/RF_fmt/cube.6"
	;;
    Tetgen-Cube-1)
...
#	mesh[3]="RF:Prysmatic-Cells/RF_fmt/vprism_16x16x16"
  ...
  mesh[6]="RF:Cubic-Cells/RF_fmt/gcube_48x48x48"
  mesh[7]="RF:Cubic-Cells/RF_fmt/gcube_64x64x64"
	;;
    Voro-Tets-2)
...
######
## Perform transformations and move meshes in outputs
...
. ../directories.sh
pathexec=$root/build/src/Mesh;

echo "Mesh family            : $mesh_family"

outsubdir=${outdir}/${mesh_family}-transformed
if [ ! -d $outsubdir ]; then
    mkdir -p $outsubdir
else
    \rm -r $outsubdir/*
...
do
  meshnamepath=$(echo ${mesh[$i]} | cut -d ':' -f 2);
  meshfile=$meshdir/$meshnamepath
  echo $meshname
  echo -e "------------------------------------------------------------------------------"
  ...
  echo -e "\n******** Execute ${trans[1]} ********\n"
  $pathexec/${trans[1]} -m $meshfile -o tmp;
  
  for j in `seq 2 $nbtrans`;
  do
    echo -e "\n******** Execute ${trans[$j]} ********\n"
    $pathexec/${trans[$j]} -m tmp -o tmp;
  done
  
  # Move mesh
  meshname=${meshnamepath##*/}
  for file in $(ls tmp.*); do
    ext=${file##*.}
    mv $file $outsubdir/$meshname.$ext
  done
done
