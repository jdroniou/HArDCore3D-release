...
# Directories
executable_name="sddr-magnetostatics"
...
echo -e "Deg MeshSize NbCells NbFaces NbEdges DimSXCurl DimSXDiv SizeSCsystem HcurlHdivError Rate" > $outsubdir/$errorsfile
...
do
    ...
    DimSXDiv=$(awk '/DimSXDiv:/ {print $NF}' $outsubdir/results-$i.txt)
    ...
    if(($i > 1)); then
      ...
      echo -e "$Degree $MeshSize $NbCells $NbFaces $NbEdges $DimSXCurl $DimSXDiv $SizeSCsystem $HcurlHdivError $HcurlHdivErrorRate" >> $outsubdir/$errorsfile
    else
  	  echo -e "$Degree $MeshSize $NbCells $NbFaces $NbEdges $DimSXCurl $DimSXDiv $SizeSCsystem $HcurlHdivError -- " >> $outsubdir/$errorsfile
    fi
done;
