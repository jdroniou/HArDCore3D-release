...
# Directories
executable_name="magnetostatics"
...
echo -e "Degree MeshSize NbCells NbFaces NbEdges DimXCurl DimXDiv EnergyError EnergyErrorRate" > $outsubdir/$errorsfile
for i in `seq 1 $nbmesh`; 
do
    ...
    if(($i \> 1)); then
      ...
      echo -e "$Degree $MeshSize $NbCells $NbFaces $NbEdges $DimXCurl $DimXDiv $EnergyError $EnergyErrorRate" >> $outsubdir/$errorsfile
  	  ...
  	  echo -e "$Degree $MeshSize $NbCells $NbFaces $NbEdges $DimXCurl $DimXDiv $EnergyError --" >> $outsubdir/$errorsfile
    fi
done;
