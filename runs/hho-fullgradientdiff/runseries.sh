...
do
    ...
    echo -e "------------------------------------------------------------------------------"
    # Execute code
    if($executable -m $meshfile -k $k -s $tcsol -x $stab_par --solver $solver); then
      # Move outputs
...
echo -e "Deg MeshSize NbCells NbFaces DimHHOSpace SizeSystem L2error Rate H1error Rate EnergyError Rate" > $outsubdir/$errorsfile
...
do
    ...
    if(($i > 1)); then
      ...
      echo -e "$Degree $MeshSize $NbCells $NbFaces $DimHHOSpace $SizeSystem $L2Error $L2ErrorRate $H1Error $H1ErrorRate $EnergyError $EnergyErrorRate" >> $outsubdir/$errorsfile
    else
  	  echo -e "$Degree $MeshSize $NbCells $NbFaces $DimHHOSpace $SizeSystem $L2Error -- $H1Error -- $EnergyError -- " >> $outsubdir/$errorsfile
    fi
done;
