...
# Test case 
# (if >0, IC, source and BC are set from this test case; if <0, only the IC is set, source and BC remain at 0) 
tcsol=1

# Imposing exact constraints on initial conditions (leads to inconsistent initial conditions if the continuous ones do not satisfy the constraint)
constr_IC=false
...
# Theta parameter of theta-scheme (1=implicit, 0.5=Crank-Nicolson)
theta=1.

# Stopping criterion for Newton iterations
stop=1e-6

# Nonlinear coeff (should be left at 1, varying it is mostly for testing purposes; 0 gives back Maxwell)
nc=1.
...
# Nonlinear discretisation method (0=xdiv bracket, 1=integral bracket)
nl_disc=1
