...
XGrad::LocalOperators XGrad::_compute_face_gradient_potential(size_t iF)
{
  ...
  auto div_ROkp2_F_quad = evaluate_quad<Divergence>::compute(*faceBases(iF).RolyComplkp2, quad_2kpo_F);
  Eigen::MatrixXd MPF = compute_gram_matrix(div_ROkp2_F_quad, basis_Pkpo_F_quad, quad_2kpo_F);
  ...
  for (size_t iE = 0; iE < F.n_edges(); iE++) {
    ...
    Eigen::MatrixXd BPF_E
      = F.edge_orientation(iE) * compute_gram_matrix(basis_ROkp2_nFE_E_quad, basis_Pkpo_E_quad, quad_2kp2_E) * edgeOperators(E).potential;
...
XGrad::LocalOperators XGrad::_compute_cell_gradient_potential(size_t iT)
{
  ...
  // NOTE: The computation of the potential is commented out since the space ROk+2
  ...
  // auto div_ROkp2_T_quad = evaluate_quad<Divergence>::compute(*cellBases(iT).RolyComplkp2, quad_2kpo_T);
  // Eigen::MatrixXd MPT = compute_gram_matrix(div_ROkp2_T_quad, basis_Pkpo_T_quad, quad_2kpo_T);
  ...
  //   auto basis_ROkp2_nF_F_quad
  //     = scalar_product(evaluate_quad<Function>::compute(*cellBases(iT).RolyComplkp2, quad_2kp2_F), T.face_normal(iF));
  ...
  //   BPT += T.face_orientation(iF) * compute_gram_matrix(basis_ROkp2_nF_F_quad, basis_Pkpo_F_quad, quad_2kp2_F) * PF;
  // } // for iF                                              

  return LocalOperators(GT, Eigen::MatrixXd::Zero(1,1)); // MPT.partialPivLu().solve(BPT));
}
...
Eigen::MatrixXd XGrad::buildGradientComponentsCell(size_t iT) const
{
  ...
  if (m_ddr_core.degree() > 0) {
    ...
    for (size_t iF = 0; iF < T.n_faces(); iF++) {
      ...
      auto basis_ROk_F_quad = evaluate_quad<Function>::compute(*m_ddr_core.faceBases(F.global_index()).RolyComplk, quad_2k_F);
    
      Eigen::MatrixXd mass_Rkmo_F = compute_gram_matrix(basis_Rkmo_F_quad, quad_2k_F);
      Eigen::MatrixXd mass_ROk_F = compute_gram_matrix(basis_ROk_F_quad, quad_2k_F);
      ...
      Eigen::MatrixXd pi_ROk_GF_F = mass_ROk_F.ldlt().solve(compute_gram_matrix(basis_ROk_F_quad, basis_Pk2_F_quad, quad_2k_F) * GF);
      ...
      uGT.block(offset, 0, PolynomialSpaceDimension<Face>::RolyCompl(degree()), dim_xgrad_T) = pi_ROk_GF_F;
    ...
    auto basis_ROk_T_quad = evaluate_quad<Function>::compute(*m_ddr_core.cellBases(iT).RolyComplk, quad_2k_T);
    
    Eigen::MatrixXd mass_Rkmo_T = compute_gram_matrix(basis_Rkmo_T_quad, quad_2k_T);
    Eigen::MatrixXd mass_ROk_T = compute_gram_matrix(basis_ROk_T_quad, quad_2k_T);
    ...
    Eigen::MatrixXd pi_ROk_GT_T = mass_ROk_T.ldlt().solve(compute_gram_matrix(basis_ROk_T_quad, basis_Pk3_T_quad, quad_2k_T) * cellOperators(iT).gradient);
    ...
    uGT.block(offset, 0, PolynomialSpaceDimension<Cell>::RolyCompl(degree()), dim_xgrad_T) = pi_ROk_GT_T;
