...
DDRCore::CellBases DDRCore::_construct_cell_bases(size_t iT)
{
  ...
  bases_T.Polykpo.reset( new PolyCellBasisType(l2_orthonormalize(basis_Pkpo_T, quad_2kpo_T, on_basis_Pkpo_T_quad)) );   
  ...
  bases_T.Polyk.reset( new RestrictedBasis<PolyCellBasisType>(*bases_T.Polykpo, PolynomialSpaceDimension<Cell>::Poly(m_K)) );  
  if (PolynomialSpaceDimension<Cell>::Poly(m_K - 1) > 0) {
    bases_T.Polykmo.reset( new RestrictedBasis<PolyCellBasisType>(*bases_T.Polykpo, PolynomialSpaceDimension<Cell>::Poly(m_K - 1)) );
  }
  ...
  PolyCellBasisType on_basis_Pk_T = l2_orthonormalize(basis_Pk_T, quad_2k_T, on_basis_Pk_T_quad);    
  // We obtain the basis for Pk(T)^3 by tensorization
  bases_T.Polyk3.reset( new Poly3CellBasisType(on_basis_Pk_T) );
  // Check that we got the dimension right
  ...
  if (PolynomialSpaceDimension<Cell>::Goly(m_K - 1) > 0) {
    ...
    bases_T.Golykmo.reset( new GolyCellBasisType(l2_orthonormalize(basis_Gkmo_T, quad_2kmo_T, basis_Gkmo_T_quad)) );
    // Check that we got the dimension right
  ...
  GolyCellBasisType on_basis_Gkpo_T = l2_orthonormalize(basis_Gkpo_T, quad_2kpo_T, basis_Gkpo_T_quad);

  // Generate a basis for GOk+1(T) by computing the kernel of MGpo
  Poly3CellBasisType basis_Pkpo3(*bases_T.Polykpo);
  Eigen::MatrixXd MGpo = compute_gram_matrix(basis_Gkpo_T_quad, on_basis_Pkpo_T_quad, quad_2kpo_T);
  bases_T.GolyOrthkpo.reset( new GolyOrthCellBasisType(basis_Pkpo3, MGpo.fullPivLu().kernel().transpose()));
  // Check that we got the dimension right
  assert( bases_T.GolyOrthkpo->dimension() == PolynomialSpaceDimension<Cell>::GolyOrth(m_K + 1) );

  if (PolynomialSpaceDimension<Cell>::GolyOrth(m_K) > 0) {
    ...
    GolyCellBasisType on_basis_Gk_T = l2_orthonormalize(basis_Gk_T, quad_2k_T, basis_Gk_T_quad);
    ...
    bases_T.GolyOrthk.reset( new GolyOrthCellBasisType(*bases_T.Polyk3, MG.fullPivLu().kernel().transpose()) );
    // Generate a basis for Rk-1(T) by taking the curl of functions in GOk(T)
    bases_T.Rolykmo.reset ( new RolyCellBasisType(*bases_T.GolyOrthk) );
    // Check that we got the dimensions right
    assert( bases_T.GolyOrthk->dimension() == PolynomialSpaceDimension<Cell>::GolyOrth(m_K) );
  ...
  if (PolynomialSpaceDimension<Cell>::RolyOrth(m_K) > 0) {
    ...
    // Finally, obtain a basis for ROk(T) by computing the kernel of the matrix MR
    Eigen::MatrixXd MR = compute_gram_matrix(spanning_set_Rk_T_quad, on_basis_Pk_T_quad, quad_2k_T);
    bases_T.RolyOrthk.reset( new RolyOrthCellBasisType(*bases_T.Polyk3, MR.fullPivLu().kernel().transpose()) );
    // Check that we got the dimension right
    assert ( bases_T.RolyOrthk->dimension() == PolynomialSpaceDimension<Cell>::RolyOrth(m_K) );
  } // if
...
DDRCore::FaceBases DDRCore::_construct_face_bases(size_t iF)
{
  ...
  // Orthonormalize and store the basis
  bases_F.Polykpo.reset( new PolyFaceBasisType(l2_orthonormalize(basis_Pkpo_F, quad_2kpo_F, basis_Pkpo_F_quad)) );
  // Check that we got the dimension right
  ...
  bases_F.Polyk.reset( new RestrictedBasis<PolyFaceBasisType>(*bases_F.Polykpo, PolynomialSpaceDimension<Face>::Poly(m_K)) );
  if (PolynomialSpaceDimension<Face>::Poly(m_K - 1) > 0) {
    bases_F.Polykmo.reset( new RestrictedBasis<PolyFaceBasisType>(*bases_F.Polykpo, PolynomialSpaceDimension<Face>::Poly(m_K - 1)) );
  }
  //------------------------------------------------------------------------------
  // Basis for Pk(F)^2
  //------------------------------------------------------------------------------
  ...
  PolyFaceBasisType on_basis_Pk_F = l2_orthonormalize(basis_Pk_F, quad_2k_F, basis_Pk_F_quad);
  // Vectorize and store the basis
  bases_F.Polyk2.reset( new Poly2FaceBasisType(on_basis_Pk_F, basis_Pk_F.jacobian()) );
  // Check that we got the dimension right
  ...
  if (PolynomialSpaceDimension<Face>::Roly(m_K - 1) > 0) {
    bases_F.Rolykmo.reset( new RolyFaceBasisType(ShiftedBasis<PolyFaceBasisType>(on_basis_Pk_F, 1)) );
    // Check that we got the dimension right
  ...
  if (PolynomialSpaceDimension<Face>::RolyOrth(m_K) > 0) {
    auto basis_Pk2_F_quad = evaluate_quad<Function>::compute(*bases_F.Polyk2, quad_2k_F);
    RolyFaceBasisType basis_Rk_F(ShiftedBasis<PolyFaceBasisType>(*bases_F.Polykpo, 1));
    ...
    bases_F.RolyOrthk.reset( new RolyOrthFaceBasisType(*bases_F.Polyk2, MR.fullPivLu().kernel().transpose()) );
    // Check that we got the dimension right
    assert ( bases_F.RolyOrthk->dimension() == PolynomialSpaceDimension<Face>::RolyOrth(m_K) );
  }
  ...
  bases_F.RolyOrthkp2.reset( new PotentialTestFunctionBasisType(l2_orthonormalize(basis_ROkp2_F, quad_2kp2_F, basis_ROkp2_F_quad)) );
  // Check that we got the dimension right
  assert( bases_F.RolyOrthkp2->dimension() == PolynomialSpaceDimension<Face>::RolyOrth(m_K + 2) );
