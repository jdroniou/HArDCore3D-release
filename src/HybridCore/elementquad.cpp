...
ElementQuad::ElementQuad(const HybridCore& hcore, const size_t iT, const size_t doeT, const size_t doeF )
  ...
  // Create face quadrature and values of basis/gradient at nodes
  size_t nfaces = m_T.n_faces();

  m_quadF.resize(nfaces);
  m_phiT_quadF.resize(nfaces);
  m_phiF_quadF.resize(nfaces);
  m_dphiT_quadF.resize(nfaces);
  for (size_t ilF = 0; ilF < nfaces ; ilF++){
    Face* face = m_T.face(ilF);
    const size_t iF = face->global_index();
    const auto& basisF = m_hcore.FaceBasis(iF);
    m_quadF[ilF] = generate_quadrature_rule(*face, m_doeF);
...
{
  const size_t n_scalar_basis = DimPoly<Face>(degree);
