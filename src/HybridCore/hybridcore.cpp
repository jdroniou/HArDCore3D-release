...
HybridCore::HybridCore(const Mesh* mesh_ptr, const int cell_deg, const size_t face_deg, const int edge_deg, const bool use_threads, std::ostream & output)
  : m_mesh(mesh_ptr),
    m_cell_deg(cell_deg),
...
HybridCore::PolyFaceBasisType HybridCore::_construct_face_basis(size_t iF)
{
  ...
  // orthonormalize and return
  return PolyFaceBasisType(l2_orthonormalize(basis_mono_F, quadF, basis_mono_F_quadF));
}
