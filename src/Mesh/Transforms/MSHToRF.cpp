...
{
    if (argc == 1){
      std::cout << "Convertes a .msh file into RF format (two files: .node, .ele).\n Usage: msh-to-rf <name of mesh>" << std::endl;
      exit(0);
    }
    ...
    if (!mshfile)
    {
        std::cerr << "     Unable to open .msh file\n";
        exit(1);
    }
...
//    }
    ...
    // File to write RF
    //------
    size_t extension = mesh_file.find_last_of("."); 
    std::string filename_core = mesh_file.substr(0, extension).substr(mesh_file.find_last_of("/\\") + 1);
    
    std::string filename_node = filename_core + ".node";
    std::string filename_ele = filename_core + ".ele";
    ...
    outNode << "# " << filename_node.c_str() << " created from " << filename_core + ".msh" << std::endl;
